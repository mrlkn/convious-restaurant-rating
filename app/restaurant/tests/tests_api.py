...
class RestaurantTestCase(APITestCase):
    def setUp(self):
        ...
        self.restaurant1 = Restaurant.objects.create(name='Restaurant 1', description='Description 1')
        self.restaurant2 = Restaurant.objects.create(name='Restaurant 2', description='Description 2')
    ...
    def test_restaurant_order_by_rating_date_range(self):
        ...
        self.assertEqual(response.data[0]['uuid'], str(self.restaurant2.uuid))

        # Test with date_from set to today
        response = self.client.get(url, {'date_from': timezone.now().strftime('%Y-%m-%d')})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]['uuid'], str(self.restaurant1.uuid))

        # Test with both date_to and date_from set to today
        response = self.client.get(url, {'date_to': timezone.now().strftime('%Y-%m-%d'),
        ...
        self.assertEqual(response.data[0]['uuid'], str(self.restaurant1.uuid))
        self.assertEqual(response.data[1]['uuid'], str(self.restaurant2.uuid))
